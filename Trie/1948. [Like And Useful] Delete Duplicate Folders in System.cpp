...
// use hashing int. same runtime. but space is good.
class Solution {
    vector<vector<string>> res;
    typedef unsigned long long ull;
    const int P = 129; // since ascii is 128
public:
    struct trie_node{
        string name;
        map<string, trie_node*> next;
    ...
    unordered_map<ull, trie_node*> seen;
    void dedupe(trie_node* root){ // post-order, similar to LC 652
        string subFolder = "";
        for(auto &[name, next]: root->next){
            dedupe(next);
            subFolder += name + "," + to_string(next->hash);
        }
        for(char c: subFolder)
            root->hash = root->hash * P + c;
        if(root->next.size()){ // not leaf
            if(seen.count(root->hash))
                seen[root->hash]->isDel = root->isDel = true;
            else
                seen[root->hash] = root;
        }
    }
