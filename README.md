# Leetcode
Solved Question: 552 / 1569

Max Rating: 2008

This is me. A newbie. 
https://leetcode.com/louisfghbvc/
```
├─Array
│      1002. Find Common Characters.cpp
│      1051. Height Checker.cpp
│      1160. Find Words That Can Be Formed by Characters.cpp
│      1329. Sort the Matrix Diagonally.cpp
│      1356. Sort Integers by The Number of 1 Bits.cpp
│      1365. How Many Numbers Are Smaller Than the Current Number.cpp
│      1366. [Like And Useful]Rank Teams by Votes.cpp
│      1375. [Like]Bulb Switcher III.cpp
│      1380. Lucky Numbers in a Matrix.cpp
│      1385. Find the Distance Value Between Two Arrays.cpp
│      1386. [Like]Cinema Seat Allocation.cpp
│      1389. Create Target Array in the Given Order.cpp
│      1394. Find Lucky Integer in an Array.cpp
│      1395. Count Number of Teams.cpp
│      1399. Count Largest Group.cpp
│      1409. Queries on a Permutation With Key.cpp
│      1413. Minimum Value to Get Positive Step by Step Sum.cpp
│      1424. Diagonal Traverse II.cpp
│      1431. Kids With the Greatest Number of Candies.cpp
│      1437. Check If All 1's Are at Least Length K Places Away.cpp
│      1441. Build an Array With Stack Operations.cpp
│      1450. Number of Students Doing Homework at a Given Time.cpp
│      1460. Make Two Arrays Equal by Reversing Sub-arrays.cpp
│      1464. Maximum Product of Two Elements in an Array.cpp
│      1465. Maximum Area of a Piece of Cake After Horizontal and Vertical Cuts.cpp
│      1470. Shuffle the Array.cpp
│      1475. Final Prices With a Special Discount in a Shop.cpp
│      1476. Subrectangle Queries.cpp
│      1480. Running Sum of 1d Array.cpp
│      1486. XOR Operation in an Array.cpp
│      1491. Average Salary Excluding the Minimum and Maximum Salary.cpp
│      1502. Can Make Arithmetic Progression From Sequence.cpp
│      1512. Number of Good Pairs.cpp
│      1528. Shuffle String.cpp
│      1529. [Like] Bulb Switcher IV.cpp
│      1534. Count Good Triplets.cpp
│      1535. Find the Winner of an Array Game.cpp
│      1550. Three Consecutive Odds.cpp
│      1560. Most Visited Sector in a Circular Track.cpp
│      1566. Detect Pattern of Length M Repeated K or More Times.cpp
│      169. [Like] Majority Element.cpp
│      217. Contains Duplicate.cpp
│      238. [Like]Product of Array Except Self.cpp
│      414. Third Maximum Number.cpp
│      442. Find All Duplicates in an Array.cpp
│      448. Find All Numbers Disappeared in an Array.cpp
│      463. Island Perimeter.cpp
│      66. Plus One.cpp
│      717. 1-bit and 2-bit Characters.cpp
│      832. Flipping an Image.cpp
│      849. Maximize Distance to Closest Person.cpp
│      896. Monotonic Array.cpp
│      918. [Like And Useful] Maximum Sum Circular Subarray.cpp
│      Counting Elements.cpp
│      [Like] Perform String Shifts.cpp
│
├─backtrack
│      1079. Letter Tile Possibilities.cpp
│      1219. Path with Maximum Gold.cpp
│      1239. Maximum Length of a Concatenated String with Unique Characters.cpp
│      1284. [Useful] Minimum Number of Flips to Convert Binary Matrix to Zero Matrix.cpp
│      1286. Iterator for Combination.cpp
│      131. Palindrome Partitioning.cpp
│      1415. The k-th Lexicographical String of All Happy Strings of Length n.cpp
│      1467. [Like] Probability of a Two Boxes Having The Same Number of Distinct Balls.cpp
│      17. Letter Combinations of a Phone Number.cpp
│      212. [Like And UseFul] Word Search II.cpp
│      216. Combination Sum III.cpp
│      22. Generate Parentheses.cpp
│      357. Count Numbers with Unique Digits.cpp
│      37. [Like] Sudoku Solver.cpp
│      39. Combination Sum.cpp
│      40. Combination Sum II.cpp
│      401. Binary Watch.cpp
│      46. Permutations.cpp
│      47. Permutations II.cpp
│      52. [Like] N-Queens II.cpp
│      526. Beautiful Arrangement.cpp
│      679. [Like And Useful] 24 Game.cpp
│      77. Combinations.cpp
│      78. Subsets.cpp
│      784. Letter Case Permutation.cpp
│      79. [Like] Word Search.cpp
│      797. All Paths From Source to Target.cpp
│      89. Gray Code.cpp
│      90. Subsets II.cpp
│
├─BFS
│      103. Binary Tree Zigzag Level Order Traversal.cpp
│      107. Binary Tree Level Order Traversal II.cpp
│      1376. Time Needed to Inform All Employees.cpp
│      1377. [Like And Useful]Frog Position After T Seconds.cpp
│      279. [Like] Perfect Squares.cpp
│      515. Find Largest Value in Each Tree Row.cpp
│      967. Numbers With Same Consecutive Differences.cpp
│      994. Rotting Oranges.cpp
│
├─Binary Search
│      1237. Find Positive Integer Solution for a Given Equation.cpp
│      1482. [Like] Minimum Number of Days to Make m Bouquets.cpp
│      153. Find Minimum in Rotated Sorted Array.cpp
│      1539. Kth Missing Positive Number.cpp
│      154. Find Minimum in Rotated Sorted Array II.cpp
│      1552. Magnetic Force Between Two Balls
│      240. [Like And Useful] Search a 2D Matrix II.cpp
│      275. H-Index II.cpp
│      278. First Bad Version.cpp
│      33. [Like]Search in Rotated Sorted Array.cpp
│      349. Intersection of Two Arrays.cpp
│      35. Search Insert Position.cpp
│      436. Find Right Interval.cpp
│      528. Random Pick with Weight.cpp
│      540. Single Element in a Sorted Array.cpp
│      852. Peak Index in a Mountain Array.cpp
│
├─Bit Manipulation
│      136. Single Number.cpp
│      137. [Like And Useful]Single Number II.cpp
│      1521. [Like And Useful]Find a Value of a Mysterious Function Closest to Target.cpp
│      1542. [Like And Useful] Find Longest Awesome Substring.cpp
│      190. Reverse Bits.cpp
│      201. [Like]Bitwise AND of Numbers Range.cpp
│      231. Power of Two.cpp
│      260. [Like] Single Number III.cpp
│      342. Power of Four.cpp
│      461. Hamming Distance.cpp
│      476. Number Complement.cpp
│
├─Bitmask
│      78. Subsets.cpp
│
├─Brainteaser
│      1503. [Like] Last Moment Before All Ants Fall Out of a Plank.cpp
│
├─Concurrency
│      1115. Print FooBar Alternately.cpp
│
├─Design
│      1357. Apply Discount Every n Orders.cpp
│      1360. Number of Days Between Two Dates.cpp
│      1381. Design a Stack With Increment Operation.cpp
│      1396. Design Underground System.cpp
│      146. [Like And Useful]LRU Cache.cpp
│      1472. [Like] Design Browser History.cpp
│      155. [Like]Min Stack.cpp
│      208. [Like And Useful] Implement Trie (Prefix Tree).cpp
│      211. [Like And Useful]Add and Search Word - Data structure design.cpp
│      380. [Like] Insert Delete GetRandom O(1).cpp
│      901. [Like] Online Stock Span.cpp
│      [Like] First Unique Number.cpp
│
├─DFS
│      130. Surrounded Regions.cpp
│      1315. Sum of Nodes with Even-Valued Grandparent.cpp
│      1391. Check if There is a Valid Path in a Grid.cpp
│      1519. [Like] Number of Nodes in the Sub-Tree With the Same Label.cpp
│      1559. [Like] Detect Cycles in 2D Grid.cpp
│      200. Number of Islands.cpp
│      733. Flood Fill.cpp
│      827. [Like And Useful] Making A Large Island.cpp
│      886. [Like] Possible Bipartition.cpp
│
├─Dp
│      1000. [Like And Useful] Minimum Cost to Merge Stones.cpp
│      1025. Divisor Game.cpp
│      1035. Uncrossed Lines.cpp
│      1048. [Like] Longest String Chain.cpp
│      1143. [Like]Longest Common Subsequence.cpp
│      1155. [Like] Number of Dice Rolls With Target Sum.cpp
│      121. Best Time to Buy and Sell Stock.py
│      123. [Like And Useful] Best Time to Buy and Sell Stock III.cpp
│      1239. Maximum Length of a Concatenated String with Unique Characters.cpp
│      1262. Greatest Sum Divisible by Three.cpp
│      1320. Minimum Distance to Type a Word Using Two Fingers.cpp
│      1335. Minimum Difficulty of a Job Schedule.cpp
│      1340. Jump Game V(Bottom-up).cpp
│      1340. Jump Game V(Top-down + Memorize).cpp
│      1343. Maximum Product of Splitted Binary Tree.cpp
│      1349. [BitMask]Maximum Students Taking Exam.cpp
│      1359. Count All Valid Pickup and Delivery Options.cpp
│      1388. [Like And Useful]Pizza With 3n Slices.cpp
│      139. [Like] Word Break.cpp
│      1397. [Like And Useful]Find All Good Strings.cpp
│      140. [Like And Useful] Word Break II.cpp
│      1402. [Like]Reducing Dishes.cpp
│      1406. [Like And Useful]Stone Game III.cpp
│      1411. [Like And Useful]Number of Ways to Paint N × 3 Grid.cpp
│      1416. [Like And Useful]Restore The Array.cpp
│      1420. [Like And Useful]Build Array Where You Can Find The Maximum Exactly K Comparisons.cpp
│      1425. [Like And Useful]Constrained Subset Sum.cpp
│      1434. [Like And Useful] Number of Ways to Wear Different Hats to Each Other.cpp
│      1444. [Like And Useful] Number of Ways of Cutting a Pizza.cpp
│      1449. [Like] Form Largest Integer With Digits That Add up to Target.cpp
│      1458. [Like] Max Dot Product of Two Subsequences.cpp
│      1463. [Like And Useful]Cherry Pickup II.cpp
│      1473. [Like] Paint House III.cpp
│      1477. [Like] Find Two Non-overlapping Sub-arrays Each With Target Sum.cpp
│      1478. [Like] Allocate Mailboxes.cpp
│      1494. [Like And Useful] Parallel Courses II.cpp
│      1504. [Like] Count Submatrices With All Ones.cpp
│      1510. Stone Game IV.cpp
│      152. [Like And Useful] Maximum Product Subarray.cpp
│      1524. [Like] Number of Sub-arrays With Odd Sum.cpp
│      1531. [Like And Useful]String Compression II.cpp
│      1531. [Like And Useful]String Compression II.py
│      1537. [Like] Get the Maximum Score.cpp
│      1547. [Like] Minimum Cost to Cut a Stick.cpp
│      1553. [Like And Useful] Minimum Number of Days to Eat N Oranges.cpp
│      1563. [Like] Stone Game V.cpp
│      1569. [Like And Useful] Number of Ways to Reorder Array to Get Same BST.cpp
│      174. [Like And Useful]Dungeon Game.cpp
│      221. [Like And Useful]Maximal Square.cpp
│      264. [Like And Useful]Ugly Number II.cpp
│      309. [Like]Best Time to Buy and Sell Stock with Cooldown.cpp
│      312. [Like And Useful] Burst Balloons.cpp
│      338. Counting Bits.cpp
│      357. Count Numbers with Unique Digits.cpp
│      368. [Like And Useful] Largest Divisible Subset.cpp
│      5-longest-palindromic-substring.cpp
│      516. [Like]Longest Palindromic Subsequence.cpp
│      518. [Like]Coin Change 2.cpp
│      53. Maximum Subarray.cpp
│      62. Unique Paths.cpp
│      64. Minimum Path Sum.cpp
│      70. Climbing Stairs.cpp
│      72. [Like] Edit Distance.cpp
│      805. [Like And Useful] Split Array With Same Average.cpp
│      877. [Like And Useful]Stone Game.cpp
│      95. [Like And Useful] Unique Binary Search Trees II.cpp
│      956. [Like And Useful]Tallest Billboard.cpp
│      96. [Like] Unique Binary Search Trees.cpp
│      983. [Like] Minimum Cost For Tickets.cpp
│
├─Geometry
│      1401. Circle and Rectangle Overlapping.cpp
│      1453. [Like] Maximum Number of Darts Inside of a Circular Dartboard.cpp
│      1515. [Like And Useful] Best Position for a Service Centre.cpp
│
├─Graph
│      1042. Flower Planting With No Adjacent.cpp
│      1192. [Like And Useful]Critical Connections in a Network.cpp
│      1361. Validate Binary Tree Nodes.cpp
│      1368. [Like And UseFul]Minimum Cost to Make at Least One Valid Path in a Grid.cpp
│      1462. Course Schedule IV.cpp
│      1466. Reorder Routes to Make All Paths Lead to the City Zero.cpp
│      1489. [Like And Useful] Find Critical and Pseudo-Critical Edges in Minimum Spanning Tree.cpp
│      1514. [Like] Path with Maximum Probability.cpp
│      207. [Like And Useful] Course Schedule.cpp
│      210. Course Schedule II.cpp
│      332. [Like And Useful] Reconstruct Itinerary.cpp
│      743. [Dijkstra]Network Delay Time.cpp
│      785. Is Graph Bipartite.cpp
│      787. [Like And Useful]Cheapest Flights Within K Stops.cpp
│      802. [Like And Useful] Find Eventual Safe States.cpp
│      997. [Like] Find the Town Judge.cpp
│
├─Greedy
│      1029. [Like] Two City Scheduling.cpp
│      1046. Last Stone Weight.cpp
│      122. Best Time to Buy and Sell Stock II.cpp
│      134. [Like And Useful] Gas Station.cpp
│      1353. Maximum Number of Events That Can Be Attended.cpp
│      1354. Construct Target Array With Multiple Sums.cpp
│      1383. [Like And Useful]Maximum Performance of a Team.cpp
│      1400. [Like]Construct K Palindrome Strings.cpp
│      1405. [Like And Useful]Longest Happy String.cpp
│      1414. Find the Minimum Number of Fibonacci Numbers Whose Sum Is K.cpp
│      1481. [Like]Least Number of Unique Integers after K Removals.cpp
│      1505. [Like And Useful] Minimum Possible Integer After at Most K Adjacent Swaps On Digits.cpp
│      1518. Water Bottles.cpp
│      1520. [Like And Useful]Maximum Number of Non-Overlapping Substrings.cpp
│      1546. [Like] Maximum Number of Non-Overlapping Subarrays With Sum Equals Target.cpp
│      1558. [Like] Minimum Numbers of Function Calls to Make Target Array.cpp
│      1561. Maximum Number of Coins You Can Get.cpp
│      1567. [Like And Useful] Maximum Length of Subarray With Positive Product.cpp
│      1568. [Like And Useful] Minimum Number of Days to Disconnect Island.cpp
│      330. [Like And Useful] Patching Array.cpp
│      402. [Like] Remove K Digits.cpp
│      406. [Like] Queue Reconstruction by Height.cpp
│      435. [Like] Non-overlapping Intervals.cpp
│      55. [Like And Useful]Jump Game.cpp
│      621. [Like And Useful] Task Scheduler.cpp
│      763. [Like And Useful]Partition Labels.cpp
│      [Like]Leftmost Column with at Least a One.cpp
│
├─Hash Table
│      1044. [Like And Useful]Longest Duplicate Substring.cpp
│      1418. Display Table of Food Orders in a Restaurant.cpp
│      1452. People Whose List of Favorite Companies Is Not a Subset of Another List.cpp
│      1487. Making File Names Unique.cpp
│      1488. [Like] Avoid Flood in The City.cpp
│      30. [Like And Useful] Substring with Concatenation of All Words.cpp
│      409. Longest Palindrome.cpp
│      525. [Like And Useful]Contiguous Array.cpp
│      560. [Like And Useful]Subarray Sum Equals K.cpp
│      771. Jewels and Stones.cpp
│
├─Heap
│      1439. [Like] Find the Kth Smallest Sum of a Matrix With Sorted Rows.cpp
│      347. [Like] Top K Frequent Elements.cpp
│      703. Kth Largest Element in a Stream.cpp
│
├─Linked List
│      1367. [Like]Linked List in Binary Tree.cpp
│      143. [Like And Useful] Reorder List.cpp
│      203. Remove Linked List Elements.cpp
│      206. [Like]Reverse Linked List.cpp
│      237. Delete Node in a Linked List.cpp
│      24. [Like]Swap Nodes in Pairs.cpp
│      328. [Like] Odd Even Linked List.cpp
│      430. Flatten a Multilevel Doubly Linked List.cpp
│      445. Add Two Numbers II.cpp
│      876. [Like]Middle of the Linked List.cpp
│
├─Math
│      1103. Distribute Candies to People.cpp
│      1232. Check If It Is a Straight Line.cpp
│      1344. Angle Between Hands of a Clock.cpp
│      1362. Closest Divisors.cpp
│      1363. [Like And Useful]Largest Multiple of Three.cpp
│      1390. [Useful]Four Divisors.cpp
│      1447. Simplified Fractions.cpp
│      1492. The kth Factor of n.cpp
│      1497. [Like] Check If Array Pairs Are Divisible by k.cpp
│      1523. Count Odd Numbers in an Interval Range.cpp
│      1551. Minimum Operations to Make Array Equal.cpp
│      171. Excel Sheet Column Number.cpp
│      202. [Like And Useful]Happy Number.cpp
│      258. Add Digits.cpp
│      29. [Like]Divide Two Integers.cpp
│      367. [Like] Valid Perfect Square.cpp
│      441. Arranging Coins.cpp
│      478. Generate Random Point in a Circle.cpp
│      50. Pow(x, n).cpp
│      60. [Like And Useful]Permutation Sequence.cpp
│      7. Reverse Integer.cpp
│      728. Self Dividing Numbers.cpp
│      836. [Like] Rectangle Overlap.cpp
│      9. Palindrome Number.cpp
│
├─Prefix
│      1277. [Like] Count Square Submatrices with All Ones.cpp
│      1352. Product of the Last K Numbers.cpp
│      1371. [Like And Useful]Find the Longest Substring Containing Vowels in Even Counts.cpp
│      1403. Minimum Subsequence in Non-Increasing Order.cpp
│      1442. Count Triplets That Can Form Two Arrays of Equal XOR.cpp
│      1525. Number of Good Ways to Split a String.cpp
│      724. Find Pivot Index.cpp
│
├─Random
│      470. Implement Rand10() Using Rand7().cpp
│      497. [Like] Random Point in Non-overlapping Rectangles.cpp
│      519. Random Flip Matrix.cpp
│
├─Recursive
│      119. [Like]Pascal's Triangle II.cpp
│      21. [Like] Merge Two Sorted Lists.cpp
│      509. Fibonacci Number.cpp
│      779. K-th Symbol in Grammar.cpp
│
├─Segment Tree
│      1526. [Like And Useful] Minimum Number of Increments on Subarrays to Form a Target Array.cpp
│
├─Sliding Window
│      1004. [Like and Useful]Max Consecutive Ones III.cpp
│      1052. Grumpy Bookstore Owner.cpp
│      1358. [Like and Useful]Number of Substrings Containing All Three Characters.cpp
│      1423. [Like]Maximum Points You Can Obtain from Cards.cpp
│      1438. [Like And Useful] Longest Continuous Subarray With Absolute Diff Less Than or Equal to Limit.cpp
│      1456. Maximum Number of Vowels in a Substring of Given Length.cpp
│      1461. [Like] Check If a String Contains All Binary Codes of Size K.cpp
│      1493. [Like] Longest Subarray of 1's After Deleting One Element.cpp
│      1499. [Like And UseFul] Max Value of Equation.cpp
│      239. [Like and Useful]Sliding Window Maximum.cpp
│      438. [Like] Find All Anagrams in a String.cpp
│      567. [Like] Permutation in String.cpp
│      904. [Like] Fruit Into Baskets.cpp
│
├─Sort
│      1387. Sort Integers by The Power Value.cpp
│      1451. Rearrange Words in a Sentence.cpp
│      1471. The k Strongest Values in an Array.cpp
│      1508. Range Sum of Sorted Subarray Sums.cpp
│      1509. [Like] Minimum Difference Between Largest and Smallest Value in Three Moves.cpp
│      1536. [Like And Useful] Minimum Swaps to Arrange a Binary Grid.cpp
│      274. [Useful] H-Index.cpp
│      451. [Like] Sort Characters By Frequency.cpp
│      561. Array Partition I.cpp
│      905. Sort Array By Parity.cpp
│      912. Sort an Array.cpp
│      937. Reorder Data in Log Files.cpp
│      969. Pancake Sorting.cpp
│      973. [Like And Useful]K Closest Points to Origin.cpp
│
├─Stack
│      739. [Like] Daily Temperatures.cpp
│
├─String
│      1021. Remove Outermost Parentheses.cpp
│      1071. Greatest Common Divisor of Strings.cpp
│      1108. Defanging an IP Address.cpp
│      1170. Compare Strings by Frequency of the Smallest Character.cpp
│      1189. [Like]Maximum Number of Balloons.cpp
│      12. [Like]Integer to Roman.cpp
│      1221. Split a String in Balanced Strings.cpp
│      13. Roman to Integer.cpp
│      1347. Minimum Number of Steps to Make Two Strings Anagram.cpp
│      1370. Increasing Decreasing String.cpp
│      1374. Generate a String With Characters That Have Odd Counts.cpp
│      1392. [Like And Useful]Longest Happy Prefix.cpp
│      1404. Number of Steps to Reduce a Number in Binary Representation to One.cpp
│      1408. String Matching in an Array.cpp
│      1410. HTML Entity Parser.cpp
│      1417. Reformat The String.cpp
│      1419. [Like And Useful]Minimum Number of Frogs Croaking.cpp
│      1422. Maximum Score After Splitting a String.cpp
│      1432. Max Difference You Can Get From Changing an Integer.cpp
│      1433. Check If a String Can Break Another String.cpp
│      1436. Destination City.cpp
│      1446. Consecutive Characters.cpp
│      1455. Check If a Word Occurs As a Prefix of Any Word in a Sentence.cpp
│      1496. Path Crossing.cpp
│      1507. Reformat Date.cpp
│      151. Reverse Words in a String.cpp
│      1513. [Like] Number of Substrings With Only 1s.cpp
│      1540. [Like] Can Convert String in K Moves.cpp
│      1541. [Like] Minimum Insertions to Balance a Parentheses String.cpp
│      1544. Make The String Great.cpp
│      1545. [Like] Find Kth Bit in Nth Binary String.cpp
│      28. Implement strStr().cpp
│      344. Reverse String.cpp
│      383. Ransom Note.cpp
│      387. First Unique Character in a String.cpp
│      388. Longest Absolute File Path.cpp
│      392. Is Subsequence.cpp
│      412. Fizz Buzz.cpp
│      415. Add Strings.cpp
│      468. Validate IP Address.cpp
│      482. License Key Formatting.cpp
│      49. [Like And Useful]Group Anagrams.cpp
│      520. Detect Capital.cpp
│      551. Student Attendance Record I.cpp
│      557. Reverse Words in a String III.cpp
│      657. Robot Return to Origin.cpp
│      67. Add Binary.cpp
│      678. [Like And Useful]Valid Parenthesis String.cpp
│      709. To Lower Case.cpp
│      788. Rotated Digits.cpp
│      804. Unique Morse Code Words.cpp
│      824. Goat Latin.cpp
│      893. Groups of Special-Equivalent Strings.cpp
│      917. Reverse Only Letters.cpp
│      921. [Like] Minimum Add to Make Parentheses Valid.cpp
│      925. Long Pressed Name.cpp
│      929. Unique Email Addresses.cpp
│
├─Tree
│      100. Same Tree.cpp
│      1008. [Like And Useful]Construct Binary Search Tree from Preorder Traversal.cpp
│      104. Maximum Depth of Binary Tree.cpp
│      106. [Like And Useful] Construct Binary Tree from Inorder and Postorder Traversal.cpp
│      108. Convert Sorted Array to Binary Search Tree.cpp
│      124. [Like] Binary Tree Maximum Path Sum.cpp
│      129. Sum Root to Leaf Numbers.cpp
│      1372. [Like]Longest ZigZag Path in a Binary Tree.cpp
│      1373. [Like And Useful]Maximum Sum BST in Binary Tree.cpp
│      1382. [Like And Useful] Balance a Binary Search Tree.cpp
│      1443. [Like] Minimum Time to Collect All Apples in a Tree.cpp
│      1448. Count Good Nodes in Binary Tree.cpp
│      1457. [Like] Pseudo-Palindromic Paths in a Binary Tree.cpp
│      1483. [Like] Kth Ancestor of a Tree Node.cpp
│      1530. [Like] Number of Good Leaf Nodes Pairs.cpp
│      222. [Like] Count Complete Tree Nodes.cpp
│      226. Invert Binary Tree.cpp
│      230. [Like] Kth Smallest Element in a BST.cpp
│      404. Sum of Left Leaves.cpp
│      437. [Like And Useful] Path Sum III.cpp
│      543. Diameter of Binary Tree.cpp
│      653. Two Sum IV - Input is a BST.cpp
│      662. [Like] Maximum Width of Binary Tree.cpp
│      669. [Like] Trim a Binary Search Tree.cpp
│      700. Search in a Binary Search Tree.cpp
│      872. Leaf-Similar Trees.cpp
│      94. [Like]Binary Tree Inorder Traversal.cpp
│      98. [Like And Useful] Validate Binary Search Tree.cpp
│      987. Vertical Order Traversal of a Binary Tree.cpp
│      993. Cousins in Binary Tree.cpp
│      [Like] Check If a String Is a Valid Sequence from Root to Leaves Path in a Binary Tree.cpp
│
├─Trie
│      1032. [Like And Useful] Stream of Characters.cpp
│
├─Two Pointers
│      11. [Like And Useful]Container With Most Water.cpp
│      125. Valid Palindrome.cpp
│      1498. [Like And Useful]Number of Subsequences That Satisfy the Given Sum Condition.cpp
│      15. [Like And Useful]3Sum.cpp
│      16. [Like And Useful]3Sum Closest.cpp
│      18. [Like] 4Sum.cpp
│      27. Remove Element.cpp
│      283. [Like]Move Zeroes.cpp
│      287. [Like And UseFul] Find the Duplicate Number.cpp
│      345. Reverse Vowels of a String.cpp
│      75. Sort Colors.cpp
│      844. Backspace String Compare.cpp
│      957. [Like] Prison Cells After N Days.cpp
│      977. Squares of a Sorted Array.cpp
│      986. [Like] Interval List Intersections.cpp
│
└─Union Find
        1562. [Like] Find Latest Group of Size M.cpp
        547. Friend Circles.cpp
        952. [Like And Useful] Largest Component Size by Common Factor.cpp
```
